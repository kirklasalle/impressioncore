...
**Tags:** #.mcp\ids_mcp\development\servers\server_enhanced_clean.py #documentation #python #source_code  
...
class EnhancedIDSMCPServer:
    ...
    async def search_documents(self, query: str, max_results: int = 10, tags: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            results = self.enhanced_ids.search_documents(query, max_results=max_results)
    ...
    async def get_file_info(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get metadata
            metadata = self.file_metadata.get(normalized_path, {})
    ...
    async def semantic_search(self, query: str, category: str = None, max_results: int = 10) -> Dict[str, Any]:
        ...
        try:
            # Use enhanced IDS semantic search if available
            if hasattr(self.enhanced_ids, 'semantic_search'):
                results = self.enhanced_ids.semantic_search(
                    query, 
                    category=category,
                    max_results=max_results
                )
            else:
                # Fallback to regular search with category filtering
                results = self.enhanced_ids.search_documents(query, max_results=max_results)
                if category:
                    results = [r for r in results if category.lower() in r.get('path', '').lower()]
...
async def handle_search(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return await server.search_documents(query, max_results, tags)
...
async def handle_list_tags(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return await server.list_tags(category, pattern)

async def handle_get_system_status(arguments: Dict[str, Any]) -> Dict[str, Any]:
    """Handle system status requests."""
    return await server.get_system_status()
...
async def handle_semantic_search(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return await server.semantic_search(query, category, max_results)
...
async def handle_search_with_context(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return await server.search_with_context(query, context_lines, max_results)

async def handle_get_search_analytics(arguments: Dict[str, Any]) -> Dict[str, Any]:
    """Handle search analytics requests."""
    days = arguments.get("days", 30)
    
    return await server.get_search_analytics(days)
...
async def handle_validate_documentation(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return await server.validate_documentation(fix_issues)
...
async def handle_generate_documentation_report(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return await server.generate_documentation_report(format)
...
async def handle_export_index_data(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return await server.export_index_data(format, include_content)
...
async def handle_create_bookmark(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return await server.create_bookmark(title, file_path, category, description, tags)
...
async def handle_manage_bookmarks(arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return await server.manage_bookmarks(action, bookmark_id, category, **kwargs)

async def handle_get_bookmark_analytics(arguments: Dict[str, Any]) -> Dict[str, Any]:
    """Handle bookmark analytics requests."""
    return await server.get_bookmark_analytics()
...
if __name__ == "__main__":
    ...
    print(f"   - Total Files: {len(server.file_metadata):,}")
    print(f"   - Total Tags: {len(server.reverse_index):,}")
    print(f"   - Total Bookmarks: {sum(len(cat) for cat in server.bookmarks_db.values()):,}")
    ...
    print(f"\n🛠️  Available Tools:")
    tools = [
        "search_documents", "find_by_tag", "get_file_info", "list_tags", "get_system_status",
        "semantic_search", "search_with_context", "get_search_analytics",
        "rebuild_index", "incremental_update", "check_index_freshness",
        "validate_documentation", "generate_documentation_report", "export_index_data",
        "create_bookmark", "manage_bookmarks", "get_bookmark_analytics"
    ]
