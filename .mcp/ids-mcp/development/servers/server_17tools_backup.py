...
**Tags:** #.mcp\ids_mcp\development\servers\server_17tools_backup.py #api #command_line #documentation #python #source_code  
...
class IDSMCPServer:
    ...
    def __init__(self, timeout=30.0):
        ...
        # Fast tag cache for performance
        self._tag_cache = None
        self._tag_cache_mtime = 0
    ...
    def get_tags_fast(self):
        """Get all tags quickly using cache."""
        try:
            reverse_index_path = DOCS_ROOT / "reverse_tag_index.yaml"
            
            # Check if we need to update cache
            if reverse_index_path.exists():
                current_mtime = reverse_index_path.stat().st_mtime
                
                if self._tag_cache is None or current_mtime > self._tag_cache_mtime:
                    logger.debug("Loading tags into cache...")
                    with open(reverse_index_path, 'r', encoding='utf-8') as f:
                        reverse_index = yaml.safe_load(f) or {}
                    
                    # Extract just the tag names (keys)
                    self._tag_cache = list(reverse_index.keys())
                    self._tag_cache_mtime = current_mtime
                    logger.debug(f"Cached {len(self._tag_cache)} tags")
            ...
            logger.error(f"Failed to get tags fast: {e}")
            return []
    ...
    async def ids_list_tags(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """List all available tags in the IDS system (optimized for speed)."""
        category = arguments.get("category", "")
        pattern = arguments.get("pattern", "")
        
        # Use fast cache method
        all_tags = self.get_tags_fast()
        ...
        # Limit to first 50 tags for speed
        display_tags = all_tags[:50]
        
        # Build fast response
        result_text = f"**{len(display_tags)} tags found"
        if len(all_tags) > 50:
            result_text += f" (showing first 50 of {len(all_tags)})"
        result_text += "**\n\n"
