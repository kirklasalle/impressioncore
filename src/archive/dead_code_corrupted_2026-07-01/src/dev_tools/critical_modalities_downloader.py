...
def download_emotion_data(base_dir):
    ...
    fer_instructions = """
# FER2013 Emotion Dataset Download Instructions
...
def download_gesture_data(base_dir):
    ...
    ntu_instructions = \"\"\"
# NTU RGB+D Gesture Dataset Access Instructions
...
def download_3d_data(base_dir):
    ...
    shapenet_instructions = \"\"\"
# 3D Model Datasets Access Instructions
...
def download_sensor_data(base_dir):
    """Download sensor and IoT datasets"""
    print("📡 Setting up Sensor/IoT Datasets...")
    sensor_dir = base_dir / "sensor_data"
    
    # TON_IoT dataset instructions
    ton_iot_instructions = \"\"\"
# TON_IoT Sensor Dataset Access

## Dataset Information
- **Source**: UNSW Research
- **Content**: IoT/IIoT sensor data, network traffic, OS logs
- **Sensors**: 10+ IoT and IIoT sensors
- **Format**: CSV and log files
- **Size**: Several GB

## Access Process
1. Visit: https://research.unsw.edu.au/projects/toniot-datasets
2. Fill out data access request form
3. Provide research/academic justification
4. Wait for approval
5. Download using provided credentials

## Alternative Sources
- **UCI ML Repository**: Various sensor datasets
- **PhysioNet**: Physiological sensor data
- **Kaggle**: Public IoT sensor datasets
\"\"\"
    
    with open(sensor_dir / "SENSOR_DATASETS_ACCESS.md", 'w') as f:
        f.write(ton_iot_instructions)
    
    # Generate comprehensive synthetic sensor data
    print("  🔋 Generating comprehensive sensor data...")
    ...
    sensor_types = {
        ...
        'pm2_5': {'min': 0, 'max': 500, 'unit': 'ug/m3'},
        'pm10': {'min': 0, 'max': 1000, 'unit': 'ug/m3'},
        'uv_index': {'min': 0, 'max': 15, 'unit': 'index'},
        'wind_speed': {'min': 0, 'max': 30, 'unit': 'm/s'}
    }
    
    # Generate data for multiple locations and sensors
    locations = ['office_1', 'office_2', 'warehouse', 'lab', 'outdoor']
    timestamps = pd.date_range(start='2024-01-01', end='2024-12-31', freq='5min')
    ...
    for location in locations:
        for sensor_type, config in sensor_types.items():
            ...
            for timestamp in timestamps:
                ...
                # Daily cycle
                daily_cycle = np.sin(hour_of_day * 2 * np.pi / 24)
                # Seasonal cycle  
                seasonal_cycle = np.sin(day_of_year * 2 * np.pi / 365)
                # Random noise
                noise = np.random.normal(0, (config['max'] - config['min']) * 0.05)
                
                # Combine patterns
                value = base_value + daily_cycle * base_value * 0.1 + seasonal_cycle * base_value * 0.2 + noise
                ...
                all_sensor_data.append({
                    ...
                    'value': round(value, 2),
                    'unit': config['unit'],
                    'status': np.random.choice(['ok', 'warning', 'error'], p=[0.9, 0.08, 0.02])
                })
    
    sensor_df = pd.DataFrame(all_sensor_data)
    sensor_df.to_csv(sensor_dir / "comprehensive_sensor_data_2024.csv", index=False)
    print(f"  ✅ Generated {len(all_sensor_data)} sensor readings")
...
def create_structured_data(base_dir):
    ...
    for account_id in range(2000):
        account_type = np.random.choice(['checking', 'savings', 'credit', 'investment'])
        customer_age = np.random.randint(18, 80)
        
        # Account characteristics based on type and age
        if account_type == 'checking':
            balance = np.random.lognormal(8, 1.5)  # Log-normal distribution
        elif account_type == 'savings':
            balance = np.random.lognormal(9, 1.8)
        elif account_type == 'credit':
            balance = -np.random.exponential(2000)  # Negative for debt
        else:  # investment
            balance = np.random.lognormal(10, 2)
        
        # Transaction patterns
        monthly_transactions = np.random.poisson(25)
        avg_transaction = balance / max(monthly_transactions, 1) * 0.1
        
        financial_data.append({
            'account_id': f"A{account_id:07d}",
            ...
            'years_with_bank': np.random.randint(0, 30),
            'default_risk': np.random.uniform(0, 1)
        })
    
    financial_df = pd.DataFrame(financial_data)
    financial_df.to_csv(structured_dir / "synthetic_financial_data.csv", index=False)
    ...
    summary = {
        "dataset_collection_summary": {
            ...
            "categories": {
                ...
                "sensor_data": {
                    "description": "IoT and environmental sensor data for context awareness",
                    "files": ["comprehensive_sensor_data_2024.csv", "SENSOR_DATASETS_ACCESS.md"],
                    "total_records": "~2.6M sensor readings",
                    "status": "✅ Generated + Real dataset instructions"
                },
                "structured_data": {
                    "description": "Tabular data for structured reasoning and analysis",
                    "files": ["synthetic_healthcare_data.csv", "synthetic_education_data.csv", "synthetic_financial_data.csv"],
                    "total_records": "8,000 records across healthcare, education, finance",
                    "status": "✅ Generated"
                }
            }
        },
        ...
        "next_steps": [
            ...
            "Request TON_IoT dataset for real-world sensor data",
        ...
        "real_world_dataset_sources": {
            "immediately_available": [
                "Weather time series (HuggingFace)",
                "Various Kaggle sensor datasets",
                "UCI ML Repository datasets"
            ],
            "requires_registration": [
                "FER2013 (Kaggle account)",
                "NTU RGB+D (research application)",
                "ShapeNet (academic registration)",
                "TON_IoT (UNSW research request)"
            ]
        }
    }
