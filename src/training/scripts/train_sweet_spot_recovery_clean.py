"""
ImpressionCore B3 Sweet Spot Recovery Training Script (Clean Version)
====================================================================
...
# Configure logging (no emojis to avoid encoding issues)
logging.basicConfig(
    level=logging.INFO,
...
class SweetSpotRecoveryTrainer:
    ...
    def __init__(self):
        ...
        self.tokenizer = None
        self.optimizer = None
        self.scaler = torch.amp.GradScaler('cuda') if self.training_config['fp16'] else None

    def setup_model(self):
        """Set up the B3 model with the exact configuration."""
        logger.info("[SETUP] Setting up B3 model with sweet spot configuration...")
        ...
        logger.info(f"[LOAD] Loading checkpoint: {self.best_quality_path}")
        if os.path.exists(self.best_quality_path):
            ...
            logger.info("[SUCCESS] Successfully loaded best quality checkpoint!")
        else:
            logger.error(f"[ERROR] Checkpoint not found: {self.best_quality_path}")
        ...
        logger.info("[SUCCESS] Model setup complete!")
    ...
    def train(self):
        """Main training loop."""
        logger.info("[TRAIN] Starting Sweet Spot Recovery Training!")
        ...
        while step < self.training_config['max_steps']:
            ...
            if step % self.training_config['log_every'] == 0:
                ...
                if torch.cuda.is_available():
                    ...
                    logger.info(f"[PROGRESS] Step {step:4d} | Loss: {avg_loss:.6f} | "
                ...
                if avg_loss < best_loss:
                    best_loss = avg_loss
                    logger.info(f"[BEST] New best loss: {best_loss:.6f}")
        ...
        logger.info(f"[COMPLETE] Training complete! Best loss: {best_loss:.6f}")
...
def main():
    ...
    logger.info("[START] ImpressionCore B3 Sweet Spot Recovery Training")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("[SUCCESS] Sweet spot recovery training completed successfully!")

    except Exception as e:
        logger.error(f"[ERROR] Training failed: {e!s}")
        raise
