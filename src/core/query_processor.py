#!/usr/bin/env python3
r"""
ImpressionCore Query Processor Module

Created: October 15, 2024
Updated: August 11, 2025 04:05:00 PM
Author: ImpressionCore Team
Tags: #cuda #memory_management #python #source_code #src\core\query_processor.py #testing
...
try:
    from src.core.utils.rich_enhancements import create_enhanced_panel  # noqa: F401
    from src.core.utils.rich_logging import get_logger
    from src.core.utils.rich_status_animation import StatusAnimation  # noqa: F401
    HAS_RICH = True
except ImportError:
    HAS_RICH = False
...
class QueryProcessor:
    ...
    def __init__(self, device: str | None = None, memory_limit_mb: int = 15):
        """
...
if __name__ == "__main__":
    ...
    logger.info("ImpressionCore Query Processor Testing")
    logger.info("=" * 60)

    for query in test_queries:
        result = processor.process_query(query)
        logger.info("Query: %s", query)
        logger.info("Intent: %s (confidence: %.2f)", result.intent.value, result.intent_confidence)
        logger.info("Entities: %s", [e.text for e in result.entities])
        logger.info("Keywords: %s", result.keywords)
        logger.info("Priority: %s", result.priority)
        logger.info("Processing time: %.1fms", result.processing_time_ms)

    # Show stats
    stats = processor.get_stats()
    logger.info("Processor Stats:")
    logger.info("Queries processed: %s", stats['queries_processed'])
    logger.info("Memory usage: %.1fMB / %sMB", stats['memory_usage_mb'], stats['memory_limit_mb'])
